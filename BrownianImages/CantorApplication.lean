...
* `CantorApplication.homogeneous_exists_profile_oscillation`: `eq:ha-asymptotic` and the
  oscillation `d_A` of `sec:concentration` put the expected profile of `μ_A` in that
  first class, with the explicit gap `d_A/2`.
* `homogeneous_application_of_endpoints`: `thm:cantor-application`, on `audit_main` and
  `audit_non_lattice_limit` as explicit hypotheses, each carrying the statement text of
  its endpoint.
* `homogeneous_application_pair_of_endpoints`: the named instance of its last sentence,
...
theorem homogeneous_application_of_endpoints {P : Measure Ω} [IsProbabilityMeasure P]
    ...
    (hmain : ∀ {P : Measure Ω} [IsProbabilityMeasure P] {W : ℝ≥0 → Ω → Plane}
      (_hW : IsPlanarBrownian W P) {s A₁ A₂ : ℝ} (_hs0 : 0 < s) (_hs1 : s < 1)
      {μ₁ μ₂ : Measure ℝ} [IsProbabilityMeasure μ₁] [IsProbabilityMeasure μ₂]
      (_h₁ : IsFrostman s A₁ μ₁) (_h₂ : IsFrostman s A₂ μ₂)
      (_hsep : ∃ ε > 0, ∀ V : ℝ, ∃ t ≥ V, ε ≤ |H s μ₁ t - H s μ₂ t|),
