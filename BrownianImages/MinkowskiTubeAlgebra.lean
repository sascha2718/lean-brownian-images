...
/-- Consequently, finite union is Borel measurable on a finite family. -/
theorem measurable_compactUnion {ι : Type*} [Fintype ι] [Nonempty ι] :
    Measurable (compactUnion : (ι → CompactPlane) → CompactPlane) :=
  continuous_compactUnion.measurable
...
/-- The elementary pointwise multiple-counting estimate, with ordered pairs. -/
theorem sum_sub_sup'_le_orderedPairMinSum {ι : Type*} [DecidableEq ι]
    {s : Finset ι} (hs : s.Nonempty) (f : ι → ℝ) (hf : ∀ i ∈ s, 0 ≤ f i) :
    (∑ i ∈ s, f i) - s.sup' hs f ≤ orderedPairMinSum s f := by
  obtain ⟨j, hj, hmax⟩ := s.exists_mem_eq_sup' hs f
  ...
  calc
    (∑ i ∈ s, f i) - s.sup' hs f = ∑ i ∈ s.erase j, f i := by
      rw [hmax, ← Finset.sum_erase_add _ _ hj]
      ring
    _ = ∑ i ∈ s.erase j, min (f i) (f j) := by
      apply Finset.sum_congr rfl
      intro i hi
      rw [min_eq_left (hle i (Finset.mem_of_mem_erase hi))]
    _ ≤ ∑ i ∈ s.erase j, ∑ k ∈ s.erase i, min (f i) (f k) := by
    ...
    _ ≤ ∑ i ∈ s, ∑ k ∈ s.erase i, min (f i) (f k) := by
      apply Finset.sum_le_sum_of_subset_of_nonneg (Finset.erase_subset j s)
      intro i his _
      exact Finset.sum_nonneg fun k hk =>
        le_min (hf i his) (hf k (Finset.mem_of_mem_erase hk))
    _ = orderedPairMinSum s f := rfl
...
/-- Pointwise, the multiple-counting defect is at most the sum of all ordered distinct-pair
overlaps. -/
theorem tubeDefectIntegrand_le_orderedTubeOverlapIntegrand {ι : Type*}
    [Fintype ι] [Nonempty ι] {r : ℝ} (hr : 0 < r) (F : ι → CompactPlane)
    (x : Plane) :
    tubeDefectIntegrand r F x ≤ orderedTubeOverlapIntegrand r F x := by
  classical
  rw [tubeDefectIntegrand, orderedTubeOverlapIntegrand,
    tubeCutoff_compactUnion hr]
  exact sum_sub_sup'_le_orderedPairMinSum Finset.univ_nonempty _ fun i _ =>
    tubeCutoff_nonneg r (F i) x
...
/-- The elementary multiple-counting estimate for smoothed tubes: the mass defect is bounded by
the sum of smoothed overlaps over ordered distinct pairs. -/
theorem tubeDefect_le_orderedSmoothedTubeOverlapSum {ι : Type*}
    [Fintype ι] [Nonempty ι] {r : ℝ} (hr : 0 < r) (F : ι → CompactPlane) :
    tubeDefect r F ≤ orderedSmoothedTubeOverlapSum r F := by
  rw [tubeDefect_eq_integral hr, ← integral_orderedTubeOverlapIntegrand hr]
  exact integral_mono (integrable_tubeDefectIntegrand hr F)
    (integrable_orderedTubeOverlapIntegrand hr F)
    (tubeDefectIntegrand_le_orderedTubeOverlapIntegrand hr F)
...
/-- The affine map used to transport a dilated and translated tube. -/
def planeAffine (a : Plane) (q : ℝ) (x : Plane) : Plane :=
  a + q • x

theorem continuous_planeAffine (a : Plane) (q : ℝ) :
    Continuous (planeAffine a q) :=
  continuous_const.add (continuous_const_smul q)
...
/-- The unnormalized cut-off integral obeys the planar affine change-of-variables formula. -/
theorem integral_tubeCutoff_affine {r q : ℝ} (hr : 0 < r) (hq : 0 < q)
    (a : Plane) (F : CompactPlane) (f : Plane → ℝ) :
    (∫ y, tubeCutoff r (translateCompact a (dilateCompact q F)) y * f y) =
      q ^ 2 * ∫ x, tubeCutoff (r / q) F x * f (planeAffine a q x) := by
  rw [← integral_add_left_eq_self
    (fun y => tubeCutoff r (translateCompact a (dilateCompact q F)) y * f y) a]
  have hpoint : ∀ y : Plane,
      tubeCutoff r (translateCompact a (dilateCompact q F)) (a + y) * f (a + y) =
        (tubeCutoff (r / q) F (q⁻¹ • y) *
          f (planeAffine a q (q⁻¹ • y))) := by
    intro y
    rw [tubeCutoff_translate, ← tubeCutoff_dilate hr hq F (q⁻¹ • y),
      smul_inv_smul₀ hq.ne' y]
    simp only [planeAffine, smul_inv_smul₀ hq.ne']
  conv_lhs =>
    enter [2, y]
    rw [hpoint y]
  rw [Measure.integral_comp_inv_smul_of_nonneg volume
    (fun x => tubeCutoff (r / q) F x * f (planeAffine a q x)) hq.le]
  have hdim : Module.finrank ℝ Plane = 2 := by simp [Plane]
  simp only [hdim, smul_eq_mul]

/-- The affine covariance identity at the level of integrals against normalized tube
probabilities. -/
theorem integral_tubeProbability_affine' {r q : ℝ} (hr : 0 < r) (hq : 0 < q)
    (a : Plane) (F : CompactPlane) (f : Plane → ℝ) :
    (∫ y, f y ∂(tubeProbability r hr
      (translateCompact a (dilateCompact q F)) : Measure Plane)) =
      ∫ x, f (planeAffine a q x)
        ∂(tubeProbability (r / q) (div_pos hr hq) F : Measure Plane) := by
  rw [integral_tubeProbability hr, integral_tubeProbability (div_pos hr hq),
    tubeMass_translate_dilate hr hq, integral_tubeCutoff_affine hr hq]
  field_simp [hq.ne']

/-- Normalized tube probability is covariant under positive dilations and translations. -/
theorem tubeProbability_affine {r q : ℝ} (hr : 0 < r) (hq : 0 < q)
    (a : Plane) (F : CompactPlane) :
    tubeProbability r hr (translateCompact a (dilateCompact q F)) =
      (tubeProbability (r / q) (div_pos hr hq) F).map
        (continuous_planeAffine a q).measurable.aemeasurable := by
  apply ProbabilityMeasure.toMeasure_injective
  apply ext_of_forall_integral_eq_of_IsFiniteMeasure
  intro f
  rw [ProbabilityMeasure.toMeasure_map,
    integral_map_of_stronglyMeasurable (continuous_planeAffine a q).measurable
      f.continuous.stronglyMeasurable]
  exact integral_tubeProbability_affine' hr hq a F f

/-- The underlying-measure form of affine covariance. -/
theorem tubeProbability_affine_toMeasure {r q : ℝ} (hr : 0 < r) (hq : 0 < q)
    (a : Plane) (F : CompactPlane) :
    (tubeProbability r hr (translateCompact a (dilateCompact q F)) : Measure Plane) =
      (tubeProbability (r / q) (div_pos hr hq) F : Measure Plane).map
        (planeAffine a q) := by
  have h := congrArg ProbabilityMeasure.toMeasure
    (tubeProbability_affine hr hq a F)
  simpa only [ProbabilityMeasure.toMeasure_map] using h

/-- Bounded continuous test functions transform by precomposition with the affine map. -/
theorem integral_tubeProbability_affine (f : Plane →ᵇ ℝ) {r q : ℝ}
    (hr : 0 < r) (hq : 0 < q) (a : Plane) (F : CompactPlane) :
    (∫ y, f y ∂(tubeProbability r hr
      (translateCompact a (dilateCompact q F)) : Measure Plane)) =
      ∫ x, f (planeAffine a q x)
        ∂(tubeProbability (r / q) (div_pos hr hq) F : Measure Plane) :=
  integral_tubeProbability_affine' hr hq a F f

/-- The same affine integral identity for a Lipschitz test function. -/
theorem integral_tubeProbability_affine_lipschitz {K : ℝ≥0} (f : Plane → ℝ)
    (hf : LipschitzWith K f) {r q : ℝ} (hr : 0 < r) (hq : 0 < q)
    (a : Plane) (F : CompactPlane) :
    (∫ y, f y ∂(tubeProbability r hr
      (translateCompact a (dilateCompact q F)) : Measure Plane)) =
      ∫ x, f (planeAffine a q x)
        ∂(tubeProbability (r / q) (div_pos hr hq) F : Measure Plane) := by
  have _hcontinuous : Continuous f := hf.continuous
  exact integral_tubeProbability_affine' hr hq a F f
