...
arithmetic theorem in `Minkowski.TubeArithmeticRenewal`.
-/
...
/-- Every lag of an arithmetic system is positive. -/
...
/-- In an arithmetic system each half-logarithmic ratio is its integer lag times the
span `h`. -/
...
/-- The renewal mass at time zero is one. -/
@[simp]
theorem renewalMass_zero : F.renewalMass 0 = 1 := by
  rw [renewalMass]

/-- The renewal recurrence: the mass at time `n + 1` is the weighted sum of the masses
one lag earlier. -/
...
/-- A sum over `k < N` restricted to `lag ≤ N - k` is a sum over `k ≤ N - lag`. -/
...
/-- The lagged convolution sum over `k < N` reindexes to a convolution over `k ≤ N -
lag`. -/
...
/-- On non-negative integers the integer-indexed renewal mass is the renewal mass. -/
theorem intRenewalMass_of_nonneg {n : ℤ} (hn : 0 ≤ n) :
    F.intRenewalMass n = F.renewalMass n.toNat := by
  simp [intRenewalMass, hn]

/-- On negative integers the integer-indexed renewal mass vanishes. -/
theorem intRenewalMass_of_neg {n : ℤ} (hn : n < 0) :
    F.intRenewalMass n = 0 := by
  simp [intRenewalMass, not_le.mpr hn]

/-- The integer-indexed renewal mass at a difference of naturals is the renewal mass at
their difference. -/
...
/-- Integer-indexed renewal masses are bounded by one. -/
