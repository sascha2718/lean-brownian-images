...
/-- Mutual-independence form of `variance_weighted_sum`. -/
theorem variance_weighted_sum_of_iIndepFun [Fintype ι] [IsFiniteMeasure μ]
    (X : ι → Ω → ℝ) (p : ι → ℝ) (hX : ∀ i, MemLp (X i) 2 μ)
    (hindep : iIndepFun X μ) :
    variance (fun ω ↦ ∑ i, p i * X i ω) μ =
      ∑ i, (p i) ^ 2 * variance (X i) μ :=
  variance_weighted_sum X p hX (pairwise_indepFun_of_iIndepFun hindep)
...
/-- Mutual-independence form of the sharp centered `L²` estimate. -/
theorem centeredL2Norm_weighted_sum_of_iIndepFun_le [Fintype ι]
    [IsFiniteMeasure μ] (X : ι → Ω → ℝ) (p a : ι → ℝ)
    (hX : ∀ i, MemLp (X i) 2 μ) (hindep : iIndepFun X μ)
    (ha0 : ∀ i, 0 ≤ a i) (ha : ∀ i, centeredL2Norm (X i) μ ≤ a i) :
    centeredL2Norm (fun ω ↦ ∑ i, p i * X i ω) μ ≤
      Real.sqrt (∑ i, (p i * a i) ^ 2) :=
  centeredL2Norm_weighted_sum_le X p a hX
    (pairwise_indepFun_of_iIndepFun hindep) ha0 ha
...
/-- In particular, the usual elementary factor-two estimate for centering holds. -/
theorem centeredL2Norm_le_two_mul_lpNorm [IsProbabilityMeasure μ] {X : Ω → ℝ}
    (hX : MemLp X 2 μ) :
    centeredL2Norm X μ ≤ 2 * lpNorm X 2 μ := by
  calc
    centeredL2Norm X μ ≤ lpNorm X 2 μ := centeredL2Norm_le_lpNorm hX
    _ ≤ 2 * lpNorm X 2 μ := by nlinarith [lpNorm_nonneg (f := X) (p := 2) (μ := μ)]
...
/-- Copy-law form of `centeredL2Norm_recurrence_le`.  Each independent input may live on
the recurrence probability space while its delayed comparison variable lives on a second
probability space. -/
theorem centeredL2Norm_recurrence_of_identDistrib_le [Fintype ι]
    [IsProbabilityMeasure μ] [IsProbabilityMeasure ν]
    (X : Ω → ℝ) (Xi : ι → Ω → ℝ) (Y : ι → Ω' → ℝ)
    (O : Ω → ℝ) (p a : ι → ℝ) (hY : ∀ i, MemLp (Y i) 2 ν)
    (hcopy : ∀ i, IdentDistrib (Xi i) (Y i) μ ν)
    (hindep : Pairwise fun i j ↦ Xi i ⟂ᵢ[μ] Xi j) (hO : MemLp O 2 μ)
    (ha0 : ∀ i, 0 ≤ a i) (ha : ∀ i, centeredL2Norm (Y i) ν ≤ a i)
    (hrec : X =ᵐ[μ] fun ω ↦ (∑ i, p i * Xi i ω) - O ω) :
    centeredL2Norm X μ ≤
      Real.sqrt (∑ i, (p i * a i) ^ 2) + lpNorm O 2 μ := by
  have hXi : ∀ i, MemLp (Xi i) 2 μ := fun i ↦ (hcopy i).memLp_iff.mpr (hY i)
  apply centeredL2Norm_recurrence_le X Xi O p a hXi hindep hO ha0
  · intro i
    rw [centeredL2Norm_eq_of_identDistrib (hcopy i) (hXi i)]
    exact ha i
  · exact hrec

/-- Mutual-independence and copy-law form of the recurrence step. -/
theorem centeredL2Norm_recurrence_of_iIndepFun_identDistrib_le [Fintype ι]
    [IsProbabilityMeasure μ] [IsProbabilityMeasure ν]
    (X : Ω → ℝ) (Xi : ι → Ω → ℝ) (Y : ι → Ω' → ℝ)
    (O : Ω → ℝ) (p a : ι → ℝ) (hY : ∀ i, MemLp (Y i) 2 ν)
    (hcopy : ∀ i, IdentDistrib (Xi i) (Y i) μ ν) (hindep : iIndepFun Xi μ)
    (hO : MemLp O 2 μ) (ha0 : ∀ i, 0 ≤ a i)
    (ha : ∀ i, centeredL2Norm (Y i) ν ≤ a i)
    (hrec : X =ᵐ[μ] fun ω ↦ (∑ i, p i * Xi i ω) - O ω) :
    centeredL2Norm X μ ≤
      Real.sqrt (∑ i, (p i * a i) ^ 2) + lpNorm O 2 μ :=
  centeredL2Norm_recurrence_of_identDistrib_le X Xi Y O p a hY hcopy
    (pairwise_indepFun_of_iIndepFun hindep) hO ha0 ha hrec
