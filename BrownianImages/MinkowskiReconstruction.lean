...
`MinkowskiTubeConvergence` proves that every term is supported in the fixed compact
...
`TubeReconstructionHypothesis` retains both analytic fields as a convenient packaged
interface, but its `imageLaw` field is now a formal consequence of `pathwise`.
Independently, `MinkowskiLimitClassifier` pulls back any Borel property of the limit
without first constructing the whole measurable limit map.
...
* `brownianImageLaw_mutuallySingular_of_tubeReconstruction`: descent of occupation-law
  singularity to compact-image-law singularity.
* `brownianImageLaw_mutuallySingular_of_pathwise_tubeReconstruction`: the stronger
  classifier route, requiring pathwise convergence alone.
* `cantor_brownianImage_application_pair` and
  `homogeneous_brownianImage_application_pair`: the paired applications.
-/
...
encapsulated by `MinkowskiLimit`. -/
...
/-- Occupation-law mutual singularity descends to compact Brownian image laws once
the tube reconstruction hypothesis holds for the two compact time sets. -/
theorem brownianImageLaw_mutuallySingular_of_tubeReconstruction
    {P : Measure Omega} {W : ℝ≥0 → Omega → Plane} (hW : IsPlanarBrownian W P)
    {K1 K2 : NonemptyCompacts ℝ} {mu1 mu2 : Measure ℝ}
    (h1 : TubeReconstructionHypothesis W P K1 mu1)
    (h2 : TubeReconstructionHypothesis W P K2 mu2)
  ...
  obtain ⟨reconstruct, hmeas, hrec1, hrec2⟩ :=
    exists_common_borel_reconstruction_of_hypotheses hW h1 h2
  apply MinkowskiTransfer.rangeLaw_mutuallySingular_of_reconstructs_occupation
    hmeas (hW.aemeasurable_brownianImage K1) (hW.aemeasurable_brownianImage K2)
    hrec1 hrec2
  exact hoccupation
...
/-- The middle-thirds homogeneous set and its paired non-arithmetic competitor have
mutually singular compact Brownian image laws, conditional only on their two packaged
tube reconstruction hypotheses. -/
theorem cantor_brownianImage_application_pair
    {P : Measure Omega} [IsProbabilityMeasure P]
    {W : ℝ≥0 → Omega → Plane} (hW : IsPlanarBrownian W P)
    {KA : Set ℝ} {muA : Measure ℝ} (hA : cantorSystem.IsNatural KA sCantor muA)
    {KB : Set ℝ} {muB : Measure ℝ}
    (hB : (pairSystem (pairRatio sCantor) (pairRatio_pos sCantor_pos)
      (pairRatio_lt_half sCantor_pos sCantor_lt_one)).IsNatural KB sCantor muB)
    (hnl : Irrational (Real.log (pairRatio sCantor)⁻¹ / Real.log 2))
    (hconvA : TubeReconstructionHypothesis W P hA.compactAttractor muA)
    (hconvB : TubeReconstructionHypothesis W P hB.compactAttractor muB) :
    (brownianImageLaw W P hA.compactAttractor).MutuallySingular
      (brownianImageLaw W P hB.compactAttractor) := by
  exact brownianImageLaw_mutuallySingular_of_tubeReconstruction hW hconvA hconvB
    (BrownianImages.cantor_application_pair hW hA hB hnl)

/-- Classifier form of the middle-thirds-versus-paired application.  Only the two
pathwise tube reconstruction assertions remain as analytic hypotheses. -/
theorem cantor_brownianImage_application_pair_of_pathwise
    {P : Measure Omega} [IsProbabilityMeasure P]
    {W : ℝ≥0 → Omega → Plane} (hW : IsPlanarBrownian W P)
    {KA : Set ℝ} {muA : Measure ℝ} (hA : cantorSystem.IsNatural KA sCantor muA)
    {KB : Set ℝ} {muB : Measure ℝ}
  ...
  exact brownianImageLaw_mutuallySingular_of_pathwise_tubeReconstruction
    hW hconvA hconvB (BrownianImages.cantor_application_pair hW hA hB hnl)

/-- Parameter-uniform homogeneous-versus-pair-system version of the compact-image
singularity corollary. -/
theorem homogeneous_brownianImage_application_pair
    {P : Measure Omega} [IsProbabilityMeasure P]
    {W : ℝ≥0 → Omega → Plane} (hW : IsPlanarBrownian W P)
