...
the renewal measure `F = ∑ p_i δ_{a_i}` alone; `PLAN.md` tracks the rest.

* `expTransform_renewalLaw`: the exponential transform of `F` in closed form.
...
* `not_nonlattice_renewalLaw`: the **refutation**.  The vendored theorem asks that `μ`
  ...
  charges one, so `F` never satisfies it, and the vendored key renewal limit does not
...
/-- `∃ θ > 0` with `∫ e^{-θz} dF < 1`: the strict exponential moment the key renewal
theorem asks for.  The log-ratios are positive and finitely many, so they are bounded
below by some `m > 0`, and `∫ e^{-z} dF ≤ e^{-m} < 1`. -/
...
/-! ### The vendored key renewal limit does not apply to `F` -/
...
`‖charFun μ t‖ < 1` for `t ≠ 0`, and for `F` that is false: at `t = 2π/(a₁ - a₀)` the two
...
and is satisfied by `F` exactly when the log-ratios are rationally independent. -/
