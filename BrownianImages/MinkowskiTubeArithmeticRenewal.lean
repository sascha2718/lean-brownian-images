...
/-- Arithmetic-periodic convergence for the mean Brownian tube profile, reduced
to the classical discrete renewal theorem.  Exponential decay of the expected
multiple-counting defect proves the full analytic uniformity; `hulim` and
`hseries` are the only remaining lattice-renewal inputs. -/
theorem System.IsNatural.meanBrownianTubeProfile_periodic_limit_of_discrete_renewal
    ...
    {c A D γ B ℓ : ℝ} (hc : 0 < c) (hD : 0 ≤ D) (hγ : 0 < γ)
    (hbounds : ∀ v : ℝ, 0 ≤ v →
      ...
      ∃ N₀ : ℕ, ∀ n : ℕ, N₀ ≤ n →
        ∀ t : Set.Icc (0 : ℝ) h,
          meanBrownianTubeProfile W P hmu.compactAttractor s
              ((t : ℝ) + (n : ℝ) * h) =
            ArithmeticRenewal.latticeSection h
              (S.halfScaleCutoffTubeRenewalForcing s
                (meanBrownianTubeProfile W P hmu.compactAttractor s))
              (S.continuous_halfScaleCutoffTubeRenewalForcing hm) u n t) :
  ...
  exact S.exists_periodic_limit_of_tubeArithmetic_of_discrete_renewal
    ...
    hc (fun v hv ↦ hbounds v hv) hB huB hulim (fun _ ↦ hseries harith)
