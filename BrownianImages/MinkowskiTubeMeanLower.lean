...
set_option linter.unusedVariables false in
/-- The same lower bound with the usual `q = 1` upper-moment hypothesis as a
single input.  Only its integrability component is needed for the lower bound. -/
theorem IsNatural.exists_expected_tubeMass_lower_of_q_one_upper
    (S : System iota) {K : Set ℝ} {s : ℝ} {mu : Measure ℝ}
    (hsep : S.IntervalSeparated) (hdim : S.IsDimension s)
    (hmu : S.IsNatural K s mu) (hs0 : 0 < s) (hs1 : s < 1)
    [IsProbabilityMeasure P] (hW : IsPlanarBrownian W P)
    (hupper : ∃ C > 0, ∀ r : ℝ, 0 < r → r ≤ 1 →
      Integrable (fun omega =>
        tubeMass r (brownianImage W hmu.compactAttractor omega)) P ∧
  ...
  exact hmu.exists_expected_tubeMass_lower_of_integrable S hsep hdim hs0 hs1 hW
